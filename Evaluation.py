...
def map2(y_true, y_pred):
    ...
    col_ind=np.array(col_ind)
    row_ind=np.array(row_ind)
    wr=np.zeros(len(y_pred))
    for i in range(len(row_ind)):
        idx = np.where(y_pred == col_ind[i])
        wr[idx] = row_ind[i]
    return wr 

def matrix(mat,matcon,a):
    figsize=(10,10)
    fig, ax = plt.subplots(figsize=figsize)
    ...
    for i in range(len(Sum1)):
        if(Sum1[i]==0):
            Sum1[i]=1
    sns.heatmap((mat.T/Sum1), square=True, annot=True, fmt='.2f',cbar=False, ax=ax,xticklabels=a, yticklabels=a,linewidths=0.5,cmap=plt.cm.Blues,)
